...
export default function BikesPage() {
  ...
  useEffect(() => {
    ...
    getVehicles({ vehicle_type: "bike" })
      ...
      .catch((e: any) => {
        if (!active) return;
        setError(e?.message || "Could not load bike companies");
      })
      .finally(() => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 pt-16 sm:pt-20 min-h-screen">
      <div className="mb-6 pb-4 border-b border-border/70">
        <h1 className="text-4xl font-extrabold tracking-tight text-foreground">Bike Companies</h1>
        <p className="text-muted-foreground mt-2 text-lg">
          Select a company to view spare parts for its bikes and scooters.
        </p>
      </div>

      {loading && <p className="text-muted-foreground">Loading bike companies...</p>}
      {error && <p className="text-destructive">{error}</p>}

      {!loading && !error && companies.length === 0 && (
        <div className="rounded-2xl border border-dashed border-border p-8 text-center text-muted-foreground">
          No bike companies found.
        </div>
      )}
        ...
        <div className="grid grid-cols-1 gap-y-8 sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-3 xl:gap-x-8">
          {companies.map((company, idx) => (
            <article
              ...
              className="group relative flex flex-col rounded-3xl border border-border bg-card/95 p-4 text-card-foreground shadow-md hover:-translate-y-1 hover:shadow-xl hover:border-bikeBlue/40 transition-all duration-300"
            >
              ...
              <div className="mt-5 flex flex-col flex-1 px-2">
                <h2 className="mb-2 text-xl font-bold text-card-foreground group-hover:text-bikeBlue transition-colors">
                  {company.company}
                </h2>
